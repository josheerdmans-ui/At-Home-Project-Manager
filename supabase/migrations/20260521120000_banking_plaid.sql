-- Banking: Plaid-linked accounts, transactions, manual investments
-- access_token lives on banking_plaid_items with no client policies (service_role only)
...
-- No client policies on banking_plaid_items (access_token stays service_role-only)
